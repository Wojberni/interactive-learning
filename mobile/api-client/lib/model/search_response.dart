...
class SearchResponse {
  ...
  @override
  String toString() => 'SearchResponse[results=$results]';
  ...
  static SearchResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      return SearchResponse(
        results: SearchResult.listFromJson(json[r'results']) ?? const [],
      );
    }
    return null;
  }
  ...
  static Map<String, List<SearchResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SearchResponse.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
